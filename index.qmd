...
**AI Disclosure:** I conceived and directed all the visuals in this explainer, which were implemented in JavaScript with the help of a coding agent. The source code for the entire project will be linked here for reference.
...
```
As can you can see, if we could somehow find the median as the pivot, the algorithm works well. In particular if we can find the pivot in $O(n)$ time where n is the size of the list, the time $T(n)$ taken so sort a list of size n is, 

$$
T(n) = 2T(\frac{n}{2}) + cn 
$$

Using master's thm (if you do not know what that is, just trust me on this):

$$
T(n) = O(nlog n)
$$

If you don't know this, $O(n log n)$ is the theoretical optimum for any comparison based sort. So this is asympotically optimal. 

The only problem is, how are we planning on always finding the median element as the pivot?

Here's the kicker - you don't have to worry about that to still get the optimal O(nlog n) time bound on average.
...
The expected running time of Randomized Quick Sort on an input of $n$ distinct elements is $O(n \log n)$.



:::
I'm going to omit the proof of this theorem here. Quicksort is really popular and the proof can be found easily. For example, 
See [Wikipedia's average-case analysis of Quicksort](https://en.wikipedia.org/wiki/Quicksort#Average-case_analysis) for a proof or CLRS Ch 7.

It's important to note that this algorithm, unlike them previous two, never fails. Given any array, it'll always give us the sorted array. 

This gives us a nice seperation of randomized algorithms in two types: Las Vegas and Monte Carlo.

::: {#def
inition-las-vegas-algorithm .definition-statement}
