...
For a randomized algorithm to work, we need two things:
...
This is hard. Even though the algorithms we discussed were simple, the way we went about dervining the failure bounds for the algorithms was not obvious. If you're inspired after reading the explainer and decide to solve a few problems on constructing a randomized algorithm, you'll immediately run into this issue. 


2. One run of the randomized algorith should be quick enough and the function that bounds the failure probability from above should vanish toward 0 quickly enough on repeated runs of the algorithm. If either of these conditions are false, the algorithm might be impractical because it'd simply take too much time to run for a good enough probability bound!
...
Let $t$ be the number of non-zero columns in $D$. Since $D \neq Q$, it contains at least one nonzero column. 
...
Compared to the Frievalds' flat bound of $\frac{1}{2}$, this absolutely sucks. It is also an extrememly slowly decreasing function, and will take a lot of amplifications to bound the probability to an acceptable amount.

For a concrete example, let's go back to multiplying two $3000 \times 3000$ matrices on a a Compaq Deskpro 386/20 having 160,000 FLOPs.
...
If you compare it with the total FLOPs for one run of Frievalds', it is significantly less ($\approx 18$ Billion vs $\approx 54$ billion).

This suggests that one iteration of this algorithm significantly faster than one iteration of Frievalds'.

However, its probability bounds suck. At $d = 300$, the probablility we fail is at most $\frac{2999}{3000} \approx 0.99667$! That's as bad as it gets.
...
The time your computer would take to carry out all $6907$ iterations is given by:
...
What went wrong with this algorithm? The probability bound $\frac{d}{d-1}$ was simply vanishing too slowly.

--

The two points above are by no means an exhaustive list over all challenges that come up when designing a randomized algorithm, but nonetheless I give you a good sense of why this is hard. 
